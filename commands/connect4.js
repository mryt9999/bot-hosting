        ...
        try {
            ...
            if (opponent.bot) {
                return await interaction.reply({
                    content: '❌ You cannot challenge a bot to Connect 4.',
            ...
            if (opponent.id === challenger.id) {
                return await interaction.reply({
                    content: '❌ You cannot challenge yourself to Connect 4.',
                    flags: [MessageFlags.Ephemeral]
                });
            }
            ...
            const challengeEmbed = new EmbedBuilder()
                ...
                .setDescription(`${challenger} has challenged ${opponent} to a game of Connect 4!`)
                .addFields(
                    { name: '💰 Bet Amount', value: `${betAmount.toLocaleString()} points each`, inline: true },
                    { name: '🔴 Red Player', value: challenger.username, inline: true },
                    { name: '🟡 Yellow Player', value: opponent.username, inline: true }
                )
                .setColor(0xE74C3C)
                .setTimestamp();
            ...
            const acceptButton = new ActionRowBuilder().addComponents(
                new ButtonBuilder()
                    .setCustomId(`c4_accept_${challenger.id}_${opponent.id}_${betAmount}`)
                    .setLabel('Accept Challenge')
                    .setStyle(ButtonStyle.Success)
                    .setEmoji('✅'),
                new ButtonBuilder()
                    .setCustomId(`c4_decline_${challenger.id}_${opponent.id}`)
