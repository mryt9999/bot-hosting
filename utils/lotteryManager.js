...
function createLotteryEmbed(lottery, ended = false) {
    ...
    if (isNumber) {
        embed.setTitle('🎲 Number Draw Lottery (1-1000)')
            .setDescription(
                ended
            ...
            .addFields(
                ...
                { name: '🎯 Numbers Used', value: `${lottery.usedNumbers.length}/${NUMBER_LOTTERY_MAX}`, inline: true },
                { name: '💳 Entry Cost', value: `${NUMBER_LOTTERY_COST} points per draw`, inline: true },
                { name: '🎰 How to Play', value: 'Click "Buy Draw" to get a random number. Check the thread below for all draws!', inline: false }
            );
        ...
        embed.setTitle('🎟️ Raffle Lottery (24 Hour)')
            .setDescription(
                ended
                    ? '**This lottery has ended.**'
            ...
            .addFields(
                ...
                { name: '💳 Entry Cost', value: `${RAFFLE_LOTTERY_COST} points (one entry)`, inline: true }
            );
        ...
        if (!ended && lottery.availableAnimals && lottery.availableAnimals.length > 0) {
            ...
            embed.addFields(
                { name: '🏁 Racing Animals', value: animalStats, inline: false }
            );
        }
        ...
        embed.addFields(
            { name: '💰 Prize Pool', value: `${lottery.prizePool.toLocaleString()} points`, inline: true },
            { name: '👥 Participants', value: `${lottery.participants.length}`, inline: true },
            { name: '💳 Entry Cost', value: `${ANIMAL_LOTTERY_COST} points (one bet)`, inline: true }
        );
        ...
        if (ended && lottery.winningAnimal) {
            ...
            if (winnerCount === 0) {
                winnerText += '💔 No one bet on the winner! Prize pool lost.';
            } else if (winnerCount === 1) {
                winnerText += `🏆 Winner: <@${lottery.winnerIds[0]}>\n💰 Prize: ${prizePerWinner.toLocaleString()} points`;
            } else {
                winnerText += `🏆 ${winnerCount} Winners!\n💰 Each won: ${prizePerWinner.toLocaleString()} points\n\n`;
...
async function archiveLottery(lottery, client, winnerId, winningNumber) {
    try {
        ...
        if (isNumber) {
            archiveEmbed.setTitle('🎲 Number Draw Lottery - Archived')
                .addFields(
                    { name: '💰 Final Prize Pool', value: `${lottery.prizePool.toLocaleString()} points`, inline: true },
                    { name: '🎯 Numbers Used', value: `${lottery.usedNumbers.length}/${NUMBER_LOTTERY_MAX}`, inline: true },
                    { name: '🏆 Winner', value: winnerId ? `<@${winnerId}>` : 'None', inline: true },
                    { name: '🎲 Winning Number', value: winningNumber ? `${winningNumber}` : 'N/A', inline: true },
                    { name: '📅 Started', value: `<t:${Math.floor(lottery.startedAt / 1000)}:F>`, inline: true },
                    { name: '🏁 Ended', value: `<t:${Math.floor((lottery.endedAt || Date.now()) / 1000)}:F>`, inline: true },
                    { name: '📊 Total Draws', value: `${Math.max(0, sortedMessages.length - 1)}`, inline: true },
                    { name: 'Lottery ID', value: `\`${lottery._id}\``, inline: false }
                );
        } else if (isRaffle) {
            archiveEmbed.setTitle('🎟️ Raffle Lottery - Archived')
                .addFields(
                    { name: '💰 Final Prize Pool', value: `${lottery.prizePool.toLocaleString()} points`, inline: true },
                    ...
                    { name: '📅 Started', value: `<t:${Math.floor(lottery.startedAt / 1000)}:F>`, inline: true },
                    { name: '🏁 Ended', value: `<t:${Math.floor((lottery.endedAt || Date.now()) / 1000)}:F>`, inline: true },
                    { name: '📊 Total Entries', value: `${Math.max(0, sortedMessages.length - 1)}`, inline: true },
                    { name: 'Lottery ID', value: `\`${lottery._id}\``, inline: false }
                );
        } else if (isAnimal) {
            const winnerCount = lottery.winnerIds ? lottery.winnerIds.length : 0;
            archiveEmbed.setTitle('🏇 Animal Race Lottery - Archived')
                .addFields(
                    { name: '💰 Final Prize Pool', value: `${lottery.prizePool.toLocaleString()} points`, inline: true },
                    { name: '👥 Total Bets', value: `${lottery.participants.length}`, inline: true },
                    { name: '🏆 Winners', value: winnerCount > 0 ? `${winnerCount}` : 'None', inline: true },
                    { name: '🏁 Winning Animal', value: lottery.winningAnimal || 'N/A', inline: true },
                    { name: '💎 Prize Per Winner', value: winnerCount > 0 ? `${Math.floor(lottery.prizePool / winnerCount).toLocaleString()} points` : 'N/A', inline: true },
                    { name: '📅 Started', value: `<t:${Math.floor(lottery.startedAt / 1000)}:F>`, inline: true },
                    { name: '🏁 Ended', value: `<t:${Math.floor((lottery.endedAt || Date.now()) / 1000)}:F>`, inline: true },
                    { name: 'Lottery ID', value: `\`${lottery._id}\``, inline: false }
                );
    ...
    if (channel) {
        ...
        try {
            const thread = await message.startThread({
                name: `🎲 Draws - ${lottery._id.toString().slice(-6)}`,
...
async function createRaffleLottery(client, guildId) {
    ...
    if (channel) {
        ...
        try {
            const thread = await message.startThread({
                name: `🎟️ Entries - ${lottery._id.toString().slice(-6)}`,
                autoArchiveDuration: 1440,
...
async function createAnimalLottery(client, guildId) {
    ...
    if (channel) {
        ...
        try {
            const thread = await message.startThread({
                name: `🏇 Race Bets - ${lottery._id.toString().slice(-6)}`,
                autoArchiveDuration: 1440,
    ...
    if (lottery.logThreadId) {
        try {
            ...
            if (thread) {
                ...
                // ✅ Use lottery's own availableAnimals for final distribution
                const distribution = lottery.availableAnimals.map(animal => {
                    const count = lottery.participants.filter(p => p.animal === animal).length;
                    const isWinner = animal === lottery.winningAnimal;
                    return `${animal} ${isWinner ? '👑' : ''}: ${count} bet${count !== 1 ? 's' : ''}`;
                }).join('\n');

                resultEmbed.addFields({
                    name: '📊 Final Betting Distribution',
                    value: distribution,
                    inline: false
                });
        ...
        try {
            ...
            if (thread) {
                const logEmbed = new EmbedBuilder()
                    .setTitle('🎉 Number Draw Lottery Winner!')
                    .setColor(0x2ECC71)
                    .addFields(
                ...
                await thread.send({
                    content: `<@${winnerId}> 🎉 **CONGRATULATIONS! YOU WON!**`,
                    embeds: [logEmbed]
                });
...
async function endRaffleLottery(lottery, client, guild) {
    if (lottery.participants.length === 0) {
        ...
        if (channel && lottery.messageId) {
            try {
                const message = await channel.messages.fetch(lottery.messageId);
                const embed = new EmbedBuilder()
                    .setTitle('🎟️ Raffle Lottery Ended')
    ...
    if (lottery.logThreadId) {
        try {
            ...
            if (thread) {
                const logEmbed = new EmbedBuilder()
                    .setTitle('🎉 Raffle Lottery Winner!')
                    .setColor(0x2ECC71)
                    .addFields(
                ...
                await thread.send({
                    content: `<@${winner.userId}> 🎉 **CONGRATULATIONS! YOU WON THE RAFFLE!**`,
                    embeds: [logEmbed]
                });
